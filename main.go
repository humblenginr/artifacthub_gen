...
func writeComponents(pkgs []artifacthub.AhPackage) error {
	...
	content, err := os.ReadFile("./components.json")
	if err == nil {
	...
	err = os.WriteFile("./components.json", val, 0666)
	if err != nil {
		return err
	}
	return nil
...
func writeExternalSystems(pkgs []artifacthub.AhPackage) error {
	...
	err = os.WriteFile("./external_systems.json", val, 0666)
	if err != nil {
		return err
	}
	return nil
...
func getPackagesFromRes(res map[string]([]map[string]interface{})) []artifacthub.AhPackage {
	...
	for _, pkg := range pkgs {
		...
		if err == nil && len(crds) != 0 {
			fmt.Println("[GetPackagesFromRes] Package with name: ", ahPkg.Name, " has been added to the list of HELM packages")
